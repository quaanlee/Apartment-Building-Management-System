...
public interface ProfileRepository extends JpaRepository<Profile, Integer> {
    ...
    @Query("SELECT p FROM Profile p LEFT JOIN p.account a LEFT JOIN a.role r WHERE " +
           "(:search IS NULL OR :search = '' OR " +
           " LOWER(p.fullName) LIKE LOWER(CONCAT('%', :search, '%')) OR " +
           " LOWER(p.email) LIKE LOWER(CONCAT('%', :search, '%')) OR " +
           " LOWER(p.phoneNumber) LIKE LOWER(CONCAT('%', :search, '%')) OR " +
           " LOWER(p.citizenId) LIKE LOWER(CONCAT('%', :search, '%')) OR " +
           " LOWER(a.username) LIKE LOWER(CONCAT('%', :search, '%'))) AND " +
